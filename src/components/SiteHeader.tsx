  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-[var(--sp-hairline)] bg-[color-mix(in_srgb,var(--color-black)_82%,transparent)] backdrop-blur-xl">
      <div className="mx-auto flex h-14 max-w-[var(--sp-max)] items-center gap-3 px-[var(--sp-gutter)] sm:gap-6">
        ...
        >
          <span className="sp-display text-[0.78rem] leading-none whitespace-nowrap tracking-[-0.02em] text-bone transition-colors group-hover:text-acid sm:text-[0.95rem]">
            {SITE.name}
          </span>
          <span className="sp-label hidden text-[0.55rem] sm:inline">EST. {SITE.founded}</span>
        </Link>

        <nav className="ml-auto flex items-center gap-0.5 sm:gap-1" aria-label="主导航">
          {NAV.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="sp-label px-1.5 py-2 text-[0.5rem] whitespace-nowrap transition-colors hover:text-acid sm:px-3 sm:text-[0.625rem]"
            >
              {item.label}
            </Link>
          ))}
